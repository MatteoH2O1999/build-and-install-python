...
describe.each(getTags())(
  'MacOSBuilder for version $version',
  ({version, zipBall}) => {
    beforeEach(async () => {
      interactions = [];
    ...
    test.each(archs)('can be instantiated for arch %s', arch => {
      const builder = new MacOSBuilder({version, zipBall}, arch, false);

      expect(builder).toBeInstanceOf(MacOSBuilder);
    ...
    test('downloads the sources from the zipball url', async () => {
      const builder = new MacOSBuilder({version, zipBall}, 'x64', false);
    ...
    if (semver.gte(version, '3.13.0')) {
      test.each(archs)(
        ...
        async arch => {
          const builder = new MacOSBuilder({version, zipBall}, arch, true);
